...
class RpjmdMisi extends Model
{
    ...
    public function tujuanTerkait(): BelongsToMany
    {
        return $this->belongsToMany(TujuanDaerah::class, 'tujuan_daerah_misi', 'rpjmd_misi_id', 'tujuan_daerah_id')
            ->withPivot('urutan')
            ->withTimestamps()
            ->orderByPivot('urutan')
            ->orderBy('tujuan_daerah.urutan');
    }
}
