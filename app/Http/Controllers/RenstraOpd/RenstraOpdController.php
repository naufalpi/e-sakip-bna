...
class RenstraOpdController extends Controller
{
    ...
    private function rpjmdReferenceOptions(int $rpjmdId): array
    {
        return [
            ...
            'program_rpjmd' => ProgramRpjmd::query()->whereHas('strategi.sasaran.tujuan', fn (Builder $query) => $query->forRpjmd($rpjmdId))->orderBy('urutan')->get(['id', 'kode', 'nama'])->map(fn ($item) => ['id' => $item->id, 'label' => $this->nodeLabel($item->kode, $item->nama)])->values()->all(),
            'indikator_program_rpjmd' => IndikatorProgramRpjmd::query()->whereHas('program.strategi.sasaran.tujuan', fn (Builder $query) => $query->forRpjmd($rpjmdId))->orderBy('urutan')->get(['id', 'kode', 'indikator'])->map(fn ($item) => ['id' => $item->id, 'label' => $this->nodeLabel($item->kode, $item->indikator)])->values()->all(),
        ];
    }
