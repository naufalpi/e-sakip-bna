...
class PerjanjianKinerjaController extends Controller
{
    ...
    public function store(StorePerjanjianKinerjaRequest $request, PerjanjianKinerjaSnapshotService $snapshotService): RedirectResponse
    {
        ...
        $pk = DB::transaction(function () use ($data, $snapshotService): PerjanjianKinerja {
    ...
    public function print(PerjanjianKinerja $perjanjianKinerja, KinerjaReportContentService $contentService): View
    {
        $this->authorize('view', $perjanjianKinerja);

        return view('reports.perjanjian-kinerja', [
            'report' => $contentService->build($perjanjianKinerja, 'perjanjian_kinerja'),
            'browserPrint' => true,
        ]);
    }
    ...
    public function update(UpdatePerjanjianKinerjaRequest $request, PerjanjianKinerja $perjanjianKinerja, PerjanjianKinerjaSnapshotService $snapshotService): RedirectResponse
    {
        $data = $request->validated();
        $this->assertRenstraBelongsToOpd($data['renstra_opd_id'] ?? null, isset($data['opd_id']) ? (int) $data['opd_id'] : null);
        $data = $this->prepareSubjectData($data);
        ...
        DB::transaction(function () use ($perjanjianKinerja, $data, $snapshotService): void {
            $sourceChanged = collect(['level_pk', 'rkpd_id', 'renstra_opd_id', 'dpa_opd_id', 'periode_tahun_id', 'tahun'])
                ->contains(fn (string $field) => (string) $perjanjianKinerja->getAttribute($field) !== (string) ($data[$field] ?? null));

            $perjanjianKinerja->update($data);

            if ($sourceChanged && in_array($perjanjianKinerja->level_pk, ['bupati', 'kepala_opd'], true)) {
    ...
    private function subjectOptions(User $user): array
    {
        $employees = Pegawai::query()
            ->where('status', 'active')
            ->when($this->shouldLimitToUserOpd($user), fn (Builder $query) => $query->where(function (Builder $query) use ($user): void {
                $query->where('opd_id', $user->opd_id)
                    ->orWhereHas('penempatan.jabatanOrganisasi', fn (Builder $query) => $query->where('level_jabatan', 'kepala_daerah'));
            }))
    ...
    private function prepareSubjectData(array $data): array
    {
        ...
        $data['tipe_pk'] = $data['level_pk'] === 'individu' ? 'individual' : 'cascading';
        $pegawai = Pegawai::query()->findOrFail($data['pegawai_id']);

        if ($data['level_pk'] !== 'bupati' && (int) $pegawai->opd_id !== (int) ($data['opd_id'] ?? 0)) {
        ...
        if ($data['atasan_pegawai_id'] ?? null) {
            ...
            $referenceDate = $data['tanggal_dokumen'] ?? sprintf('%d-01-01', $data['tahun']);
            ...
            if (! in_array($data['level_pk'], ['bupati', 'kepala_opd'], true)
                && (int) $atasan->opd_id !== (int) ($data['opd_id'] ?? 0)) {
