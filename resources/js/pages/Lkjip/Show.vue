    ...
    <div class="flex flex-col gap-4 p-4">
        <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
                <button v-if="can.export" type="button" class="rounded-md border px-3 py-2 text-sm hover:bg-muted" @click="exportDocument('pdf')">
                    Export PDF
                </button>
                <button v-if="can.export" type="button" class="rounded-md border px-3 py-2 text-sm hover:bg-muted" @click="exportDocument('word')">
                    Export Word
                </button>
                <button v-if="can.manage" type="button" class="rounded-md border px-3 py-2 text-sm hover:bg-muted" @click="generateDraft">
                    Buat Draft Otomatis
                </button>
        ...
        <section class="grid gap-3 rounded-lg border bg-card p-4 md:grid-cols-4">
            ...
            <div>
                ...
                <div class="mt-1 font-medium">
                    {{ item.evaluasi_sakip ? `${item.evaluasi_sakip.nilai_akhir ?? '-'} / ${item.evaluasi_sakip.predikat || '-'}` : '-' }}
                </div>
            </div>
        </section>
        ...
        <section class="grid gap-3 rounded-lg border bg-card p-4 md:grid-cols-2">
            <div>
                ...
                <p class="mt-3 whitespace-pre-line text-sm text-muted-foreground">
                    {{ item.ringkasan_eksekutif || 'Belum ada ringkasan eksekutif.' }}
                </p>
            </div>
            <div>
        ...
        <section class="overflow-hidden rounded-lg border bg-card">
            ...
            <div v-else class="px-4 py-8 text-center text-sm text-muted-foreground">
                Belum ada dokumen otomatis. Klik generate atau export untuk membuat dokumen melalui queue.
            </div>
        </section>
        ...
        <section v-if="can.manage" class="rounded-lg border bg-card p-4">
            <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                ...
                <button v-if="editingBabId" type="button" class="w-fit rounded-md border px-3 py-2 text-sm hover:bg-muted" @click="resetBabForm">
                    Batal Edit
                </button>
            </div>
                ...
                <article v-for="bab in item.bab" :key="bab.id" class="p-4">
                    <div class="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                        ...
                        <div v-if="can.manage" class="flex gap-2">
                            ...
                            <button type="button" class="rounded-md border px-2 py-1 text-xs text-red-700 hover:bg-red-50" @click="destroyBab(bab)">
                                Hapus
                            </button>
                        </div>
                    </div>
