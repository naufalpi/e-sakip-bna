    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-4 p-4">
            ...
            <section class="overflow-hidden rounded-lg border bg-card">
                <div class="divide-y">
                    <article v-for="notification in notifications.data" :key="notification.id" class="flex flex-col gap-2 p-4 md:flex-row md:items-start md:justify-between" :class="notification.read_at ? '' : 'bg-emerald-50/50'">
                        <div>
                            <div class="flex flex-wrap items-center gap-2">
                                <h2 class="text-sm font-semibold">{{ notification.title }}</h2>
                                <span v-if="!notification.read_at" class="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-800">Baru</span>
                            </div>
                        ...
                        <button v-if="!notification.read_at" type="button" class="w-fit rounded-md border px-3 py-1.5 text-xs hover:bg-muted" @click="markRead(notification.id)">Tandai Dibaca</button>
                    </article>
                ...
                <div class="flex flex-col gap-3 border-t px-4 py-3 text-sm text-muted-foreground md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex gap-2">
                        <Link v-if="notifications.prev_page_url" :href="notifications.prev_page_url" class="rounded-md border px-3 py-1.5 hover:bg-muted">Sebelumnya</Link>
                        ...
                        <Link v-if="notifications.next_page_url" :href="notifications.next_page_url" class="rounded-md border px-3 py-1.5 hover:bg-muted">Berikutnya</Link>
