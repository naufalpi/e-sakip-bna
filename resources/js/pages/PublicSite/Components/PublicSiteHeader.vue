...
<template>
    <header class="fixed inset-x-0 top-0 z-50 border-b border-blue-100 bg-white shadow-sm shadow-blue-950/5">
        ...
        <div class="mx-auto flex h-[4.5rem] max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
            <Link
                :href="homeUrl"
                class="flex min-h-11 items-center gap-3 transition duration-200 hover:opacity-90 focus-visible:opacity-90 focus-visible:outline-none"
            >
            ...
            <nav class="hidden items-center gap-1 lg:flex" aria-label="Navigasi utama">
                <Link
                    ...
                    :class="item.isActive ? 'text-[#00336C] after:h-0.5 after:scale-x-100 after:bg-[#00336C]' : 'text-slate-700 after:scale-x-0'"
                >
                    {{ item.label }}
                </Link>
            </nav>
            ...
            <div class="hidden items-center gap-3 lg:flex">
                <Link
                    :href="entryUrl"
                    class="inline-flex min-h-11 items-center gap-2 rounded-md bg-[#00336C] px-4 py-2 text-sm font-semibold text-white shadow-sm shadow-blue-950/15 transition duration-200 hover:-translate-y-px hover:bg-[#002957] hover:shadow-md hover:shadow-blue-950/20 focus-visible:-translate-y-px focus-visible:bg-[#002957] focus-visible:shadow-md focus-visible:shadow-blue-950/20 focus-visible:outline-none active:translate-y-0"
                >
                    <LogIn class="h-4 w-4" />
                    {{ entryLabel }}
                </Link>
