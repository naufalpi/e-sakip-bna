...
const applyFilters = () => router.get(route('dokumen.index'), filterForm, { preserveState: true, replace: true });
const resetFilters = () => {
    filterForm.search = '';
    filterForm.jenis = '';
    filterForm.status = '';
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-4 p-4">
            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                ...
                <Link v-if="can.manage" :href="route('dokumen.create')" class="inline-flex h-9 items-center justify-center gap-2 rounded-md bg-emerald-700 px-3 text-sm font-medium text-white hover:bg-emerald-800">
                    <Plus class="size-4" />
                    Unggah Dokumen
                </Link>
            </div>

            <form class="grid gap-3 rounded-lg border bg-card p-3 lg:grid-cols-[1fr_180px_170px_220px_190px_auto_auto]" @submit.prevent="applyFilters">
                <div class="relative">
                    <Search class="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
                    <input v-model="filterForm.search" type="search" class="h-9 w-full rounded-md border bg-background pl-9 pr-3 text-sm outline-none focus:ring-2 focus:ring-emerald-700" placeholder="Cari judul, nomor, atau nama file" />
                </div>
                <select v-model="filterForm.jenis" class="h-9 rounded-md border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-emerald-700">
            ...
            <div class="overflow-hidden rounded-lg border bg-card">
                        ...
                        <tbody>
                            <tr v-for="row in dokumen.data" :key="row.id" class="border-b last:border-0">
                                <td class="px-4 py-3">
                                    ...
                                    <div class="text-xs text-muted-foreground">{{ jenisLabel(row.jenis) }} - {{ row.nomor_dokumen || 'Nomor belum diisi' }}</div>
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="inline-flex gap-2">
                                        ...
                                        <Link v-if="can.manage" :href="route('dokumen.edit', row.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted">Edit</Link>
                ...
                <div class="flex flex-col gap-3 border-t px-4 py-3 text-sm text-muted-foreground md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex gap-2">
                        <Link v-if="dokumen.prev_page_url" :href="dokumen.prev_page_url" class="rounded-md border px-3 py-1.5 hover:bg-muted">Sebelumnya</Link>
                        ...
                        <Link v-if="dokumen.next_page_url" :href="dokumen.next_page_url" class="rounded-md border px-3 py-1.5 hover:bg-muted">Berikutnya</Link>
