...
const setImportFile = (event: Event) => {
    importForm.file = (event.target as HTMLInputElement).files?.[0] ?? null;
};

const submitImport = () => {
    importForm.post(route('rpjmd.import-placeholder'), {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-4 p-4">
            ...
            <form v-if="can.manage" class="grid gap-3 rounded-lg border bg-card p-3 md:grid-cols-[1fr_auto]" @submit.prevent="submitImport">
                <div>
                    <label class="mb-1 block text-sm font-medium" for="rpjmd_import">Import Excel Cascading RPJMD</label>
                    <input
                        id="rpjmd_import"
                        type="file"
                        accept=".xlsx,.xls,.csv"
                    ...
                    />
                    <p class="mt-1 text-xs text-muted-foreground">Placeholder import sudah disiapkan. Parsing detail akan dikerjakan pada tahap lanjutan.</p>
                    <p v-if="importForm.errors.file" class="mt-1 text-xs text-red-600">{{ importForm.errors.file }}</p>
                </div>
                <button
                    type="submit"
                    :disabled="importForm.processing"
                    class="inline-flex h-9 items-center justify-center gap-2 self-end rounded-md border px-3 text-sm font-medium hover:bg-muted disabled:opacity-60"
                >
            ...
            <div class="overflow-hidden rounded-lg border bg-card">
                            ...
                            <tr v-for="rpjmd in rpjmds.data" :key="rpjmd.id" class="border-b last:border-0">
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="inline-flex gap-2">
                                        ...
                                        <Link v-if="can.manage" :href="route('rpjmd.edit', rpjmd.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted">Edit</Link>
                                        <button
                                            v-if="can.manage"
                                            type="button"
                ...
                <div class="flex flex-col gap-3 border-t px-4 py-3 text-sm text-muted-foreground md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex gap-2">
                        <Link v-if="rpjmds.prev_page_url" :href="rpjmds.prev_page_url" class="rounded-md border px-3 py-1.5 hover:bg-muted">Sebelumnya</Link>
                        ...
                        <Link v-if="rpjmds.next_page_url" :href="rpjmds.next_page_url" class="rounded-md border px-3 py-1.5 hover:bg-muted">Berikutnya</Link>
