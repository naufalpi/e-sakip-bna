...
const selectedFilterLabels = computed(() =>
    [
        ...
        filterForm.opd_id ? `OPD: ${props.opdOptions.find((option) => String(option.id) === String(filterForm.opd_id))?.label ?? 'Terpilih'}` : null,
        filterForm.rpjmd_id ? `RPJMD: ${props.rpjmdOptions.find((option) => String(option.id) === String(filterForm.rpjmd_id))?.label ?? 'Terpilih'}` : null,
        filterForm.periode_tahun_id
                    ...
                    <div class="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                        <div class="min-w-0">
                            ...
                            <p class="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
                                Kelola dokumen Renstra, pantau keterhubungan RPJMD, dan lanjutkan pengisian cascading tujuan, sasaran, program,
                                kegiatan, sub kegiatan, indikator, serta target triwulan.
                            </p>
                        </div>
            ...
            <section class="rounded-lg border bg-card p-4 shadow-sm">
                ...
                >
                    ...
                    <label class="grid min-w-0 gap-1.5">
                        ...
                        <select v-model="filterForm.status" class="h-10 rounded-md border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-emerald-700">
                        ...
                        <article v-for="renstra in renstras.data" :key="`mobile-${renstra.id}`" class="rounded-lg border bg-background p-4">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    ...
                                    <h3 class="mt-1 text-base font-semibold leading-snug text-slate-950">{{ renstra.opd?.singkatan || renstra.opd?.nama || '-' }}</h3>
                                </div>
                            ...
                            <div class="mt-4 grid gap-3 rounded-md border bg-card p-3">
                                ...
                                <div class="flex flex-wrap gap-1.5">
                                    ...
                                    <span class="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">{{ renstra.tahun_awal }}-{{ renstra.tahun_akhir }}</span>
                                </div>
                            </div>
                ...
                <aside class="grid gap-4 xl:sticky xl:top-20 xl:self-start">
                    <section class="rounded-lg border bg-card p-4 shadow-sm">
                        ...
                        <ol class="mt-4 space-y-3 text-sm">
                            <li class="flex gap-3">
                                <span class="flex size-6 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-semibold text-emerald-800">1</span>
                                <span>Buat identitas Renstra dan hubungkan ke RPJMD.</span>
                            </li>
                            <li class="flex gap-3">
                                <span class="flex size-6 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-semibold text-emerald-800">2</span>
                                <span>Isi cascading tujuan, sasaran, program, kegiatan, dan sub kegiatan.</span>
                            </li>
                            <li class="flex gap-3">
                                <span class="flex size-6 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-semibold text-emerald-800">3</span>
                                <span>Lengkapi indikator serta target tahunan/triwulan.</span>
                            </li>
                            <li class="flex gap-3">
                                <span class="flex size-6 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-semibold text-emerald-800">4</span>
