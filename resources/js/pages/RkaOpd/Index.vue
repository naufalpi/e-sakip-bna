...
const statusLabel = (status: string) =>
    ({ draft: 'Draft', submitted: 'Diajukan', revision: 'Perlu Perbaikan', verified: 'Terverifikasi', approved: 'Disetujui', rejected: 'Ditolak', locked: 'Terkunci' })[status] ?? status;
...
<template>
    ...
    <div class="flex flex-col gap-5 p-4 sm:p-5">
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-card shadow-sm dark:border-slate-800">
            <div class="relative border-b border-slate-200 bg-[linear-gradient(116deg,#fff_0%,#f5f9ff_60%,#edf6ff_100%)] px-5 py-6 dark:border-slate-800 dark:bg-[linear-gradient(116deg,#0f172a_0%,#0b1d34_62%,#0a2746_100%)] sm:px-6">
                ...
                <div class="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex min-w-0 items-start gap-4">
                        <div class="flex size-12 shrink-0 items-center justify-center rounded-xl bg-[#00336C] text-white shadow-[0_8px_24px_rgba(0,51,108,.22)]">
                            <FileSpreadsheet class="size-5" />
                        </div>
                        <div>
                            <p class="text-[11px] font-bold uppercase tracking-[0.16em] text-[#5276a0] dark:text-blue-300">Penganggaran berbasis kinerja</p>
                            <h1 class="mt-1 text-2xl font-bold tracking-tight text-slate-950 dark:text-white">RKA OPD</h1>
                            <p class="mt-1 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300">Rencana Kerja dan Anggaran Satuan Kerja Perangkat Daerah.</p>
                        </div>
                    </div>
                    <Link v-if="can.manage" :href="route('rka-opd.create')" class="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#00336C] px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-[#002855] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-950">
                        <Plus class="size-4" />
                        Buat RKA dari RENJA
                    </Link>
                </div>
            ...
            <div class="grid divide-y divide-slate-200 sm:grid-cols-4 sm:divide-x sm:divide-y-0 dark:divide-slate-800">
                <div v-for="metric in [
                    { label: 'Dokumen', value: stats.documents, icon: FileSpreadsheet, tone: 'text-blue-700 bg-blue-50 dark:bg-blue-950/50 dark:text-blue-300' },
                    ...
                    { label: 'Pemeriksaan', value: stats.review, icon: ShieldCheck, tone: 'text-cyan-700 bg-cyan-50 dark:bg-cyan-950/50 dark:text-cyan-300' },
                    { label: 'Disetujui', value: stats.approved, icon: BadgeCheck, tone: 'text-emerald-700 bg-emerald-50 dark:bg-emerald-950/50 dark:text-emerald-300' },
                ]" :key="metric.label" class="flex items-center gap-3 px-5 py-4">
                    <div class="flex size-9 items-center justify-center rounded-lg" :class="metric.tone"><component :is="metric.icon" class="size-4" /></div>
                    <div><p class="text-xs text-slate-500 dark:text-slate-400">{{ metric.label }}</p><p class="text-xl font-bold tabular-nums text-slate-950 dark:text-white">{{ metric.value }}</p></div>
                </div>
            </div>
        </section>
        ...
        <section class="rounded-2xl border border-slate-200 bg-card p-4 shadow-sm dark:border-slate-800 sm:p-5">
            <div class="mb-4 flex items-center justify-between gap-3">
                <div><h2 class="font-bold text-slate-900 dark:text-white">Temukan dokumen</h2><p class="text-xs text-slate-500 dark:text-slate-400">Saring RKA berdasarkan OPD, tahun, jenis anggaran, atau status.</p></div>
                <button type="button" class="rounded-lg px-3 py-2 text-sm font-medium text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:hover:bg-slate-800 dark:hover:text-white" @click="resetFilters">Reset</button>
            </div>
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-card shadow-sm dark:border-slate-800">
            <header class="flex items-center justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-800 sm:px-6">
                <div><h2 class="font-bold text-slate-900 dark:text-white">Daftar RKA OPD</h2><p class="text-xs text-slate-500 dark:text-slate-400">{{ items.total }} dokumen ditemukan.</p></div>
                <span class="hidden items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600 sm:inline-flex dark:bg-slate-800 dark:text-slate-300"><Banknote class="size-3.5" /> Anggaran kinerja</span>
            </header>
            ...
            <div v-if="items.data.length" class="divide-y divide-slate-200 dark:divide-slate-800">
                <article v-for="row in items.data" :key="row.id" class="grid gap-4 px-5 py-5 transition hover:bg-slate-50/70 dark:hover:bg-slate-900/50 sm:px-6 xl:grid-cols-[minmax(15rem,1.15fr)_minmax(12rem,.8fr)_minmax(11rem,.7fr)_150px_auto] xl:items-center">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2"><p class="truncate font-bold text-slate-950 dark:text-white">{{ row.opd?.singkatan || row.opd?.nama || 'OPD' }}</p><span class="rounded-md border border-blue-200 bg-blue-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-blue-700 dark:border-blue-800 dark:bg-blue-950/50 dark:text-blue-200">{{ row.type_label }}</span></div>
                    ...
                    <div class="flex items-center gap-2 xl:justify-end">
                        ...
                        <Link v-if="row.can_update" :href="route('rka-opd.edit', row.id)" class="inline-flex size-10 items-center justify-center rounded-lg border text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800" title="Edit RKA"><Pencil class="size-4" /></Link>
                        <button v-if="row.can_delete" type="button" class="inline-flex size-10 items-center justify-center rounded-lg border border-red-200 text-red-600 hover:bg-red-50 dark:border-red-900 dark:hover:bg-red-950/40" title="Hapus RKA" @click="destroy(row)"><Trash2 class="size-4" /></button>
                    </div>
                </article>
            </div>
            <div v-else class="px-6 py-16 text-center"><FileSpreadsheet class="mx-auto size-10 text-slate-300" /><p class="mt-3 font-semibold text-slate-700 dark:text-slate-200">Belum ada RKA</p><p class="mt-1 text-sm text-slate-500">Buat RKA dari RENJA yang sudah ditetapkan.</p></div>
