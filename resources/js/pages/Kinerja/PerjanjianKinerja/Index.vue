...
const applyFilters = () => router.get(route('perjanjian-kinerja.index'), filterForm, { preserveState: true, replace: true });
const resetFilters = () => {
    filterForm.search = '';
    filterForm.status = '';
    filterForm.opd_id = '';
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-4 p-4">
            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                ...
                <Link v-if="can.manage" :href="route('perjanjian-kinerja.create')" class="inline-flex h-9 items-center justify-center gap-2 rounded-md bg-emerald-700 px-3 text-sm font-medium text-white hover:bg-emerald-800">
                    <Plus class="size-4" />
                    Tambah PK
                </Link>
            </div>
                            ...
                            <tr v-for="row in items.data" :key="row.id" class="border-b last:border-0">
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="inline-flex gap-2">
                                        <Link :href="route('perjanjian-kinerja.show', row.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted">Buka</Link>
                                        <Link v-if="can.manage" :href="route('perjanjian-kinerja.edit', row.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted">Edit</Link>
