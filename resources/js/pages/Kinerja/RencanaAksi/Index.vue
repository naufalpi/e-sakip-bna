...
const applyFilters = () => router.get(route('rencana-aksi.index'), filterForm, { preserveState: true, replace: true });
const resetFilters = () => {
    filterForm.search = '';
    filterForm.status = '';
    filterForm.opd_id = '';
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-4 p-4">
            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-normal">Rencana Aksi</h1>
                    <p class="mt-1 text-sm text-muted-foreground">Rencana pelaksanaan indikator kinerja OPD per periode realisasi.</p>
                </div>
                <Link v-if="can.manage" :href="route('rencana-aksi.create')" class="inline-flex h-9 items-center justify-center gap-2 rounded-md bg-emerald-700 px-3 text-sm font-medium text-white hover:bg-emerald-800">
                    <Plus class="size-4" />
                    Tambah Rencana
                </Link>
            </div>

            <form class="grid gap-3 rounded-lg border bg-card p-3 lg:grid-cols-[1fr_170px_220px_190px_120px_auto_auto]" @submit.prevent="applyFilters">
                <div class="relative">
                    <Search class="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
                    <input v-model="filterForm.search" type="search" class="h-9 w-full rounded-md border bg-background pl-9 pr-3 text-sm outline-none focus:ring-2 focus:ring-emerald-700" placeholder="Cari judul atau OPD" />
                </div>
                        ...
                        <tbody>
                            <tr v-for="row in items.data" :key="row.id" class="border-b last:border-0">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <div class="text-xs text-muted-foreground">{{ row.tahun }}</div>
                                </td>
                                <td class="px-4 py-3 text-muted-foreground">{{ row.perjanjian_kinerja?.judul || '-' }}</td>
                                <td class="px-4 py-3"><span class="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">{{ row.items_count }} aksi</span></td>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="inline-flex gap-2">
                                        <Link :href="route('rencana-aksi.show', row.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted">Buka</Link>
                                        <Link v-if="can.manage" :href="route('rencana-aksi.edit', row.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted">Edit</Link>
                            ...
                            <tr v-if="items.data.length === 0">
                                <td colspan="6" class="px-4 py-10 text-center text-muted-foreground">Belum ada data Rencana Aksi.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
