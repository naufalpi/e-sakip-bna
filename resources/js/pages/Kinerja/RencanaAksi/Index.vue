...
const props = defineProps<{
    items: Paginator<Row>;
    filters: { search?: string; status?: string; opd_id?: string; periode_tahun_id?: string; tahun?: string; per_page?: string };
    opdOptions: Option[];
...
const applyFilters = () => router.get(route('rencana-aksi.index'), filterForm, { preserveState: true, preserveScroll: true, replace: true });
...
const destroy = async (row: Row) => {
    if (await confirmDelete(`Hapus Rencana Aksi ${row.tahun} - ${row.judul}?`)) {
        router.delete(route('rencana-aksi.destroy', { rencana_aksi: row.id }));
    }
};
...
const formatClass = (version?: number) =>
    Number(version || 1) >= 2
        ? 'border-sky-200 bg-sky-50 text-sky-800 dark:border-sky-800 dark:bg-sky-950/35 dark:text-sky-200'
...
const groupedItems = computed<OpdGroup[]>(() => {
    ...
    props.items.data.forEach((row) => {
        const key = row.opd ? `opd-${row.opd.id}` : 'opd-tidak-diketahui';
        if (!groups.has(key)) {
            groups.set(key, {
                key,
                opd: row.opd ?? null,
                name: row.opd?.nama ?? 'Perangkat daerah belum ditentukan',
                shortName: row.opd?.singkatan || row.opd?.nama || 'Tanpa OPD',
                code: row.opd?.kode || 'Kode OPD belum tersedia',
                rows: [],
                officialCount: 0,
            });
        }
        ...
        group.rows.push(row);
        if (['approved', 'locked'].includes(row.status)) group.officialCount += 1;
    });

    return Array.from(groups.values()).sort((a, b) => a.name.localeCompare(b.name, 'id-ID'));
});

const visibleOfficial = computed(() => props.items.data.filter((row) => ['approved', 'locked'].includes(row.status)).length);
</script>

<template>
    <Head title="Rencana Aksi" />
    <main class="ra-index p-4 lg:p-6">
        <header class="ra-hero">
            <div class="relative z-10 flex min-w-0 items-start gap-4">
                <div class="hero-mark"><ClipboardCheck class="size-6" /></div>
                <div class="min-w-0">
                    <p class="text-[11px] font-extrabold uppercase tracking-[0.2em] text-primary">Pelaksanaan Kinerja</p>
                    <h1 class="mt-1 text-2xl font-bold tracking-tight lg:text-[1.7rem]">Rencana Aksi</h1>
                    <p class="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">
                        Matriks pelaksanaan kinerja ditata per perangkat daerah agar keterkaitan PK, target triwulan, dan penanggung jawab mudah dipantau.
                    </p>
                </div>
            </div>
            <div class="relative z-10 flex flex-wrap items-center gap-2 sm:gap-3">
                <div class="hero-stat">
                    ...
                    <span><strong>{{ items.total }}</strong><small>Total RA</small></span>
                </div>
                <div class="hero-stat">
                    <Building2 class="size-4 text-primary" />
                    <span><strong>{{ groupedItems.length }}</strong><small>Grup tampil</small></span>
                </div>
                <div class="hero-stat">
                    <CheckCircle2 class="size-4 text-emerald-600" />
                    <span><strong>{{ visibleOfficial }}</strong><small>Resmi tampil</small></span>
                </div>
                <Link v-if="can.manage" :href="route('rencana-aksi.create')" class="primary-action">
                    <Plus class="size-4" /> Buat Rencana Aksi
                </Link>
            </div>
        </header>
        ...
        <form class="filter-panel" @submit.prevent="applyFiltersNow">
            <div class="filter-search">
                ...
                <input v-model="filterForm.search" type="search" placeholder="Cari dokumen Rencana Aksi atau OPD" />
            </div>
            ...
            <section v-for="group in groupedItems" :key="group.key" class="opd-section">
                <header class="opd-heading">
                    ...
                    <div class="opd-summary">
                        <span><strong>{{ group.rows.length }}</strong> dokumen</span><i aria-hidden="true"></i>
                        <span><strong>{{ group.officialCount }}</strong> resmi</span>
                    </div>
                </header>
                    ...
                    <article v-for="row in group.rows" :key="row.id" class="document-row">
                        <div class="document-main">
                            <div class="mb-2 flex flex-wrap items-center gap-2">
                                <span class="format-badge" :class="formatClass(row.format_version)">
                                    {{ Number(row.format_version || 1) >= 2 ? 'Matriks kinerja' : 'Versi lama' }}
                                </span>
                                <span class="status-badge" :class="statusClass(row.status)">{{ statusLabel(row.status) }}</span>
                            </div>
                            <h3>{{ row.judul }}</h3>
                            <p>Rencana Aksi Tahun {{ row.tahun }}</p>
                        </div>

                        <div class="document-meta">
                            <div class="meta-cell meta-source">
                                <span class="meta-label"><FileCheck2 class="size-3.5" /> PK acuan</span>
                                <strong>{{ row.perjanjian_kinerja?.judul || 'PK belum terhubung' }}</strong>
                                <small>{{ row.perjanjian_kinerja ? `Tahun ${row.perjanjian_kinerja.tahun}` : 'Dokumen versi lama' }}</small>
                            </div>
                            <div class="meta-cell">
                                <span class="meta-label"><CalendarDays class="size-3.5" /> Periode</span>
                                <strong>{{ row.tahun }}</strong>
                                <small>{{ row.periode_tahun?.nama || 'Periode tahunan' }}</small>
                            </div>
                            <div class="meta-cell">
                                <span class="meta-label"><FileText class="size-3.5" /> Isi matriks</span>
                                <strong>{{ row.items_count }} {{ Number(row.format_version || 1) >= 2 ? 'baris' : 'aksi' }}</strong>
                                <small>{{ Number(row.format_version || 1) >= 2 ? 'Indikator cascading' : 'Struktur lama' }}</small>
                            </div>
                        </div>

                        <div class="document-actions">
                            <Link :href="route('rencana-aksi.show', { rencana_aksi: row.id })" class="open-action">
                                Buka <ArrowRight class="size-4" />
                            </Link>
                            <Link
                                v-if="can.manage"
                                :href="route('rencana-aksi.edit', { rencana_aksi: row.id })"
                                class="icon-action"
        ...
        <div v-else class="empty-state">
            <div class="empty-mark"><ClipboardCheck class="size-7" /></div>
            <h2>Belum ada Rencana Aksi</h2>
            <p>Ubah filter pencarian atau buat Rencana Aksi dari PK Kepala OPD resmi.</p>
            <Link v-if="can.manage" :href="route('rencana-aksi.create')" class="primary-action mt-4">
                <Plus class="size-4" /> Buat Rencana Aksi
            </Link>
        </div>

        <div class="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-card shadow-sm dark:border-slate-800">
            <DataPagination v-model:per-page="filterForm.per_page" :paginator="items" item-label="dokumen Rencana Aksi" />
        </div>
    </main>
</template>
...
.filter-panel {
    display: grid;
    grid-template-columns: minmax(260px, 1.5fr) minmax(150px, 0.65fr) minmax(220px, 1fr) minmax(180px, 0.8fr) 100px auto;
    gap: 0.55rem;
    margin-block: 1rem;
    border: 1px solid hsl(var(--border));
    border-radius: 0.85rem;
...
.document-meta {
    ...
    min-width: 0;
    flex: 1 1 47%;
    grid-template-columns: minmax(180px, 1.45fr) minmax(100px, 0.7fr) minmax(120px, 0.85fr);
    gap: 0.45rem;
}
.meta-cell {
    display: grid;
...
@media (max-width: 1240px) {
    .filter-panel {
        grid-template-columns: minmax(240px, 1.4fr) repeat(3, minmax(145px, 0.8fr));
    }
    .document-row {
