...
type Row = {
    ...
    perjanjian_kinerja?: { judul: string } | null;
    rencana_aksi?: { judul: string } | null;
};
type Paginator<T> = {
    data: T[];
    current_page: number;
...
const props = defineProps<{
    items: Paginator<Row>;
    filters: { search?: string; status?: string; opd_id?: string; periode_tahun_id?: string; tahun?: string; periode_realisasi?: string; triwulan?: string };
    opdOptions: Option[];
...
const filterForm = reactive({
    ...
    periode_realisasi: props.filters.periode_realisasi ?? '',
...
const destroy = (row: Row) => {
    if (confirm(`Hapus Realisasi Kinerja ${row.tahun}?`)) {
        router.delete(route('realisasi-kinerja.destroy', row.id));
    }
};
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-4 p-4">
            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-normal">Realisasi Kinerja</h1>
                    <p class="mt-1 text-sm text-muted-foreground">Pelaporan realisasi indikator, capaian, kendala, dan tindak lanjut.</p>
                </div>
                <Link v-if="can.manage" :href="route('realisasi-kinerja.create')" class="inline-flex h-9 items-center justify-center gap-2 rounded-md bg-emerald-700 px-3 text-sm font-medium text-white hover:bg-emerald-800">
                    <Plus class="size-4" />
                    Tambah Realisasi
                </Link>
            </div>
            ...
            <form class="grid gap-3 rounded-lg border bg-card p-3 xl:grid-cols-[1fr_160px_210px_180px_110px_150px_120px_auto]" @submit.prevent="applyFiltersNow">
                ...
                <input v-model="filterForm.tahun" type="number" class="h-9 rounded-md border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-emerald-700" placeholder="Tahun" />
                <select v-model="filterForm.periode_realisasi" class="h-9 rounded-md border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-emerald-700">
                ...
                <select v-model="filterForm.triwulan" class="h-9 rounded-md border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-emerald-700">
                        ...
                        <tbody>
                            <tr v-for="row in items.data" :key="row.id" class="border-b last:border-0">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <div class="text-xs text-muted-foreground">{{ row.periode_realisasi }} {{ row.triwulan || row.bulan || row.semester || '' }}</div>
                                </td>
                                <td class="px-4 py-3 text-muted-foreground">
                                    <div>{{ row.perjanjian_kinerja?.judul || '-' }}</div>
                                    <div class="text-xs">{{ row.rencana_aksi?.judul || '-' }}</div>
                                </td>
                                <td class="px-4 py-3"><span class="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">{{ row.programs_count }} indikator</span></td>
                                <td class="px-4 py-3"><span class="inline-flex rounded-full px-2 py-1 text-xs font-medium" :class="statusClass(row.status)">{{ statusLabel(row.status) }}</span></td>
                                <td class="px-4 py-3 text-right">
                                    <div class="inline-flex gap-2">
                                        <Link :href="route('realisasi-kinerja.show', row.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted">Buka</Link>
                                        <Link v-if="can.manage" :href="route('realisasi-kinerja.edit', row.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted">Edit</Link>
                                        <button v-if="can.manage" type="button" class="rounded-md border px-2 py-1 text-xs text-red-700 hover:bg-red-50" @click="destroy(row)">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                            <tr v-if="items.data.length === 0">
                                <td colspan="6" class="px-4 py-10 text-center text-muted-foreground">Belum ada data Realisasi Kinerja.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
