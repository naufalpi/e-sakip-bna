...
const destroy = async (row: Row) => {
    if (await confirmDelete(`Hapus Realisasi Kinerja ${row.tahun}?`)) {
...
<template>
    ...
    <div class="flex flex-col gap-4 p-4">
        <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            ...
            <Link
                v-if="can.manage"
                :href="route('realisasi-kinerja.create')"
        ...
        <form
            class="grid gap-3 rounded-lg border bg-card p-3 xl:grid-cols-[1fr_160px_210px_180px_110px_150px_120px_auto]"
        ...
        >
            <div class="relative">
                ...
                <input
                    ...
                    placeholder="Cari OPD atau catatan"
                />
            </div>
            <select
            ...
            />
            <select
                v-model="filterForm.periode_realisasi"
                class="h-9 rounded-md border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-emerald-700"
            >
                        ...
                        <tr v-for="row in items.data" :key="row.id" class="border-b last:border-0">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <div class="text-xs text-muted-foreground">
                                    {{ row.periode_realisasi }} {{ row.triwulan || row.bulan || row.semester || '' }}
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <span class="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">{{ row.programs_count }} indikator</span>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex gap-2">
                                    <Link :href="route('realisasi-kinerja.show', row.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted"
                                        >Buka</Link
                                    >
                                    <Link
                                        v-if="can.manage"
                                        :href="route('realisasi-kinerja.edit', row.id)"
