...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Revisi Target', href: '/target-revisions' },
];
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-4 p-4">
            <div class="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                <div>
                    ...
                    <p class="mt-1 max-w-3xl text-sm text-muted-foreground">
                        Riwayat pengajuan perubahan target tahunan/triwulan yang sudah disetujui atau terkunci. Perubahan nilai hanya diterapkan setelah reviewer menyetujui pengajuan.
                    </p>
                </div>
                ...
                <form class="grid gap-2 md:grid-cols-[160px_180px_1fr_auto_auto]" @submit.prevent="applyFilters">
                    ...
                    <select v-model="filterForm.module" class="h-9 rounded-md border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-emerald-700">
                        ...
                        <tbody>
                            <tr v-for="row in revisions.data" :key="row.id" class="border-b last:border-0">
                                <td class="min-w-[220px] px-4 py-3 align-top">
                                    ...
                                    <div class="mt-1 text-xs text-muted-foreground">Diajukan {{ row.requested_by || '-' }} pada {{ formatDateTime(row.created_at) }}</div>
                                </td>
                                ...
                                <td class="px-4 py-3 text-right align-top">
                                    <div v-if="can.review && row.status === 'submitted'" class="flex justify-end gap-2">
                                        <button type="button" class="inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs text-emerald-800 hover:bg-emerald-50" @click="openReview(row.id, 'approve')">
                                            <CheckCircle2 class="size-3.5" />
                                            Setujui
                                        </button>
                                        <button type="button" class="inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs text-red-700 hover:bg-red-50" @click="openReview(row.id, 'reject')">
            ...
            <div class="flex items-center justify-between text-sm">
                <Link v-if="revisions.prev_page_url" :href="revisions.prev_page_url" class="rounded-md border px-3 py-1.5 hover:bg-muted">Sebelumnya</Link>
                <span v-else class="rounded-md border px-3 py-1.5 opacity-50">Sebelumnya</span>
                <Link v-if="revisions.next_page_url" :href="revisions.next_page_url" class="rounded-md border px-3 py-1.5 hover:bg-muted">Berikutnya</Link>
            ...
            <div v-if="activeAction" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                <form class="w-full max-w-lg rounded-lg bg-background p-4 shadow-lg" @submit.prevent="submitReview">
                    ...
                    <p class="mt-1 text-sm text-muted-foreground">
                        {{ activeAction.action === 'approve' ? 'Nilai target akan langsung diperbarui setelah disetujui.' : 'Catatan penolakan wajib diisi agar pengusul mengetahui alasan penolakan.' }}
                    </p>
                    <textarea v-model="noteForm.note" rows="4" class="mt-4 w-full rounded-md border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-700" placeholder="Catatan reviewer" />
                    ...
                    <div class="mt-4 flex justify-end gap-2">
                        ...
                        <button type="submit" :disabled="noteForm.processing" class="rounded-md bg-emerald-700 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-800 disabled:opacity-60">
