...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Unit OPD', href: '/master/opd-units' },
];
...
const applyFilters = () => router.get(route('master.opd-units.index'), filterForm, { preserveState: true, replace: true });
const resetFilters = () => {
    filterForm.search = '';
    filterForm.status = '';
    filterForm.opd_id = '';
        ...
        <div class="flex flex-col gap-4 p-4">
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-muted-foreground">Struktur unit kerja internal OPD untuk penanggung jawab rencana aksi dan bukti dukung.</p>
                </div>
                <Link v-if="can.manage" :href="route('master.opd-units.create')" class="inline-flex h-9 items-center justify-center gap-2 rounded-md bg-emerald-700 px-3 text-sm font-medium text-white hover:bg-emerald-800">
                    <Plus class="size-4" />
                    Tambah Unit
                </Link>
            </div>

            <form class="grid gap-3 rounded-lg border bg-card p-3 md:grid-cols-[1fr_auto_auto_auto_auto_auto]" @submit.prevent="applyFilters">
                <div class="relative">
                ...
                <select v-model="filterForm.jenis_unit" class="h-9 rounded-md border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-emerald-700">
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody>
                            <tr v-for="item in items.data" :key="item.id" class="border-b last:border-0">
                                ...
                                <td class="px-4 py-3">
                                    <span class="inline-flex rounded-full px-2 py-1 text-xs font-medium" :class="item.status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700'">
                                        {{ item.status === 'active' ? 'Aktif' : 'Tidak aktif' }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-right">
                                    <div v-if="can.manage" class="inline-flex gap-2">
                                        <Link :href="route('master.opd-units.edit', item.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted">Edit</Link>
