...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Satuan Indikator', href: '/master/satuan-indikator' },
];
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-4 p-4">
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                ...
                <Link v-if="can.manage" :href="route('master.satuan-indikator.create')" class="inline-flex h-9 items-center justify-center gap-2 rounded-md bg-emerald-700 px-3 text-sm font-medium text-white hover:bg-emerald-800">
                    <Plus class="size-4" />
                    Tambah Satuan
                </Link>
            </div>
                            ...
                            <tr v-for="item in items.data" :key="item.id" class="border-b last:border-0">
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div v-if="can.manage" class="inline-flex gap-2">
                                        <Link :href="route('master.satuan-indikator.edit', item.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted">Edit</Link>
