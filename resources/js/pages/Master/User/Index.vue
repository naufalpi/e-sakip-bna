    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-4 p-4">
            ...
            <form class="grid gap-3 rounded-lg border bg-card p-3 md:grid-cols-[1fr_180px_240px_auto_auto]" @submit.prevent="applyFilters">
                ...
                <select v-model="filterForm.role" class="h-9 rounded-md border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-emerald-700">
            ...
            <div class="overflow-hidden rounded-lg border bg-card">
                            ...
                            <tr v-for="user in users.data" :key="user.id" class="border-b last:border-0">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex max-w-md flex-wrap gap-1">
                                        <span v-for="role in user.roles" :key="role.id" class="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">
                                            {{ role.label }}
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div v-if="can.create" class="inline-flex gap-2">
                                        <Link :href="route('master.users.edit', user.id)" class="rounded-md border px-2 py-1 text-xs hover:bg-muted">Edit</Link>
                                        <button
                ...
                <div class="flex flex-col gap-3 border-t px-4 py-3 text-sm text-muted-foreground md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex gap-2">
                        <Link v-if="users.prev_page_url" :href="users.prev_page_url" class="rounded-md border px-3 py-1.5 hover:bg-muted">Sebelumnya</Link>
                        ...
                        <Link v-if="users.next_page_url" :href="users.next_page_url" class="rounded-md border px-3 py-1.5 hover:bg-muted">Berikutnya</Link>
