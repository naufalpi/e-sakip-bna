...
const mainMetrics = computed(() => [
    {
        ...
        value: formatPercent(props.stats.avg_capaian),
    ...
    },
    {
        ...
        value: formatScore(props.stats.avg_evaluasi),
        helper: `${props.stats.evaluasi_opd_count} OPD dievaluasi`,
        icon: FileCheck2,
    },
    {
        label: 'Realisasi OPD',
        value: `${props.stats.realisasi_opd_count}/${props.stats.opd_count}`,
    ...
    <div class="flex h-full flex-1 flex-col gap-5 p-4">
        ...
        <section v-if="quickLinks.length" class="flex flex-wrap gap-2">
            <Link
                ...
                :key="link.href"
                :href="link.href"
                class="inline-flex h-9 items-center gap-2 rounded-md border bg-background px-3 text-sm hover:bg-muted"
            >
                {{ link.label }}
                    ...
                    <div v-for="row in efficiencyStatusDistribution" :key="row.status" class="space-y-1.5">
                        ...
                        <div class="h-2 overflow-hidden rounded-full bg-muted">
                            <div
                                ...
                                :class="{ 'bg-blue-600': row.status === 'cukup_efisien', 'bg-red-600': row.status === 'tidak_efisien' }"
                                :style="{ width: `${row.percent}%` }"
                            />
                        </div>
                    </div>
        ...
        <section class="grid gap-4 xl:grid-cols-2">
                            ...
                            <tr v-for="row in achievementIndicatorDrilldown" :key="row.id" class="border-b last:border-0">
                                <td class="px-4 py-3">
                                    ...
                                    <div class="mt-1 flex flex-wrap gap-2 text-xs text-muted-foreground">
                                        <Link v-if="row.opd_detail_url" :href="row.opd_detail_url" class="hover:text-emerald-800 hover:underline">{{
                                            row.opd || '-'
                                        }}</Link>
            ...
            <div class="rounded-lg border bg-card">
                ...
                <div class="divide-y">
                    <div
                        v-for="row in opdPerformanceRanking"
                        :key="row.opd_id"
                        class="grid grid-cols-[44px_1fr_92px] items-center gap-3 px-4 py-3 text-sm"
                    >
                        ...
                        <div class="min-w-0">
                            <Link
                                ...
                                class="truncate font-medium text-emerald-800 hover:underline"
                            >
                        ...
                        <tr v-for="row in progressOpd" :key="row.opd_id" class="border-b last:border-0">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <div class="mt-2 flex flex-wrap gap-2 text-xs">
                                    ...
                                    <Link v-if="row.rencana_aksi_url" :href="row.rencana_aksi_url" class="text-emerald-800 hover:underline"
                                        >Rencana Aksi</Link
                                    >
                ...
                <div class="space-y-3 p-4">
                    <div v-for="row in achievementByYear" :key="row.tahun" class="grid grid-cols-[70px_1fr_90px] items-center gap-3 text-sm">
                        ...
                        <div class="h-3 overflow-hidden rounded-full bg-muted">
                            <div
                                class="h-full rounded-full"
                                :class="row.selected ? 'bg-emerald-700' : 'bg-slate-400'"
