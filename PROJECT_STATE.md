...
- Verifikasi terakhir:
    ...
    - optimasi buka RENSTRA: suite `RenstraOpdTest` lulus 26 test/531 assertion, ESLint halaman Kelola RENSTRA bersih, PHP Pint lulus, dan build production Vite lulus.
...
- Performa buka RENSTRA dioptimalkan tanpa cache respons halaman: rekap kelengkapan lima tahap digabung menjadi satu query `UNION ALL`, sedangkan kamus referensi form, master, satuan, dan workflow hanya dimuat pada halaman yang benar-benar memakainya. Halaman Kelola Tujuan/Sasaran/Program/Kegiatan/Sub Kegiatan sekarang hanya memuat kedalaman pohon, indikator, dan kandidat master yang diperlukan oleh bagiannya; pagu Program/Kegiatan dihitung lewat agregat database tanpa menghidrasi seluruh turunan. Kandidat Kegiatan/Sub Kegiatan dibatasi ke master induk RENSTRA bila pemetaannya lengkap, dengan fallback katalog penuh agar data lama tetap dapat diperbaiki. Dampak penghapusan cascading dihitung akurat melalui endpoint berotorisasi dan `no-store` hanya saat pengguna menekan hapus; penghapusan dibatalkan bila pemeriksaan dampak gagal. Komponen workflow, dialog konfirmasi, serta kalkulasi frontend lama yang tidak terpakai tidak lagi membebani initial load. Ringkasan tetap membaca seluruh hasil filter dan preview umum tetap memperoleh pohon lengkap.
- Navigasi halaman Inertia mempertahankan perilaku standar tanpa layar penuh `Menyiapkan halaman…`: halaman saat ini tetap terlihat pada posisi scroll pengguna sampai respons tujuan siap, lalu langsung diganti. Animasi keluar yang sempat mempertahankan DOM lama setelah reset scroll telah dihapus sehingga daftar RENSTRA tidak berkedip kembali ke posisi atas ketika membuka detail maupun bagian Tujuan/Sasaran/Program/Kegiatan/Sub Kegiatan. Indikator progres tipis global tetap tersedia untuk request non-prefetch. Tautan daftar/detail RENSTRA memakai prefetch saat hover dengan cache singkat 5 detik.
...
- Label indikator pada form Sub Kegiatan RENJA menjadi `Indikator Sub Kegiatan`. Nama satuan dari RENSTRA ditampilkan readonly di bawah indikator. Seluruh kolom nilai kinerja pada preview tabel RENJA—Target Akhir RENSTRA, Realisasi Tahun Lalu, Prakiraan Tahun Berjalan, Target RENJA, dan Prakiraan Maju—menampilkan nilai beserta **nama satuan**, bukan simbol satuan, serta tidak menambahkan nama satuan lagi bila nilainya sudah diakhiri satuan yang sama.
